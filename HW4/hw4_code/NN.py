...
class dlnet:
    ...
    def Leaky_Relu(self,alpha, u):
        ...
        '''
        leakyRelu_copy = np.copy(u)

        return np.maximum(leakyRelu_copy, alpha * leakyRelu_copy)
    ...
    def _dropout(u, prob):
        ...
        for i in range(u.shape[0]):
            piece = np.random.choice(2, u.shape[1], p = [prob, 1 - prob])
            dropout_mask.append(piece)
        u_after_dropout = (np.multiply(dropout_mask, u)) * (1 / (1 - prob))
    ...
    def forward(self, x, use_dropout):
        ...
        u2 = self.param['theta2'] @ self.ch['o1'] + self.param['b2'] # IMPLEMENT THIS LINE
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        dLoss_b2 = np.sum(dLoss_u2, axis = 1) # IMPLEMENT THIS LINE
        dLoss_o1 = np.transpose(np.dot(dLoss_u2.T, self.param['theta2']))  # IMPLEMENT THIS LINE
        ...
        dLoss_b1 = np.sum(dLoss_u1, axis = 1, keepdims = True) # IMPLEMENT THIS LINE
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        for layer in dLoss:
            if use_momentum:
                ...
                self.param[layer] = self.param[layer] - (self.lr * self.change[layer]) # IMPLEMENT THIS LINE
            else:
    ...
    def gradient_descent(self, x, y, iter = 60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            ...
            losses = self.nloss(y, yh)

            if local_test == False and i % 1000 == 0:
