...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_idx = np.random.choice(range(num_training), size = num_training, replace = True)

        #################################################################################

        ############# Get Col Indices Second - write your code below ####################
        col_idx = np.random.choice(range(num_features), size = int(self.max_features * num_features), replace = False)
    ...
    def select_hyperparameters(self):
        ...
        n_estimators = 4 #Hint: Consider values between 3-15.
