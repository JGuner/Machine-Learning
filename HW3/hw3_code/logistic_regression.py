...
class LogisticRegression(object):
    ...
    def bias_augment(self, x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        x_aug = np.concatenate([temp, x], axis = 1)
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float: # [3pts]
        ...
        """
        points = y.shape[0]
        loss = -1 / points * np.sum(y * np.log(h_x) + (1 - y) * np.log(1 - h_x))
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray: # [3pts]
        ...
        grad = np.dot(x_aug.T, p) / y.shape[0]
    ...
    def fit(
        ...
        for e in range(epochs):
            h_x = self.sigmoid(np.dot(x_aug, theta))
            grad = self.gradient(x_aug, y_train, h_x)
            theta -= grad * lr
            
            if e % 100 == 0:
                self.update_evaluation_lists(x_train, y_train, x_val, y_val, theta, e)
        ########################
        ### END STUDENT CODE ###
        ########################
        return theta
