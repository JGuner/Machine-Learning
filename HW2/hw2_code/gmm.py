...
class GMM(object):
    ...
    def softmax(self, logit):  # [5pts]
        ...
        expLogit = np.exp(logit - np.max(logit, axis = 1, keepdims = True))
        sumLogit = np.sum(expLogit, axis = 1, keepdims = True)
        prob = expLogit / sumLogit
        return prob
    ...
    def logsumexp(self, logit):  # [5pts]
        ...
        expLogit = np.exp(logit - np.max(logit, axis = 1, keepdims = True))
        sumLogit = np.sum(expLogit, axis = 1, keepdims = True)
        s = np.log(sumLogit) + np.max(logit, axis = 1, keepdims = True)
        return s
    ...
    def normalPDF(self, points, mu_i, sigma_i):  # [5pts]
        ...
        pdf = np.prod((1/np.sqrt(2*np.pi*np.diagonal(sigma_i))) * np.exp((-1/(2 * np.diagonal(sigma_i))) * np.square(points - mu_i)), axis = 1)
        return pdf
    ...
    def _init_components(self, **kwargs):  # [5pts]
        ...
        pi = np.zeros(self.K) + np.array(1 / self.K)
        mu = np.zeros((self.K, self.D))
        sigma = np.zeros((self.K, self.D, self.D))
        pointAxis = self.points.shape[0]
        randomPoint = np.random.choice(pointAxis, size = self.K, replace = False)
        for i in range(len(sigma)):
            mu[i] = self.points[int(randomPoint[i])]
            sigma[i] = np.eye(self.D)
        return pi, mu, sigma
    ...
    def _ll_joint(self, pi, mu, sigma, full_matrix=FULL_MATRIX, **kwargs):  # [10 pts]
        ...
        if (full_matrix == False):
            for i in range(self.K):
                logPi = np.log(pi[i] + LOG_CONST)
                log_normalPDF = np.log((self.normalPDF(self.points, mu[i], sigma[i])) + LOG_CONST)
                ll[:, i] = logPi + log_normalPDF
        return ll
    ...
    def _E_step(self, pi, mu, sigma, full_matrix = FULL_MATRIX , **kwargs):  # [5pts]
        ...
        # === undergraduate implementation
        if (full_matrix == False):
            gamma = self.softmax(self._ll_joint(pi, mu, sigma, full_matrix))
        return gamma
    ...
    def _M_step(self, gamma, full_matrix=FULL_MATRIX, **kwargs):  # [10pts]
        ...
        if (full_matrix == False):
            ...
            sigma = np.ones((self.K, self.D, self.D))
            pi = np.sum(gamma, axis = 0) / self.points.shape[0]
            for i in range(self.K):
                mu[i] = np.dot(gamma[:, i].T, self.points) / np.sum(gamma[:, i], axis = 0)
                diff = (self.points - mu[i])
                gammaDot = np.dot(np.multiply(gamma[:, i], diff.T), diff)
                gammaTransposeSum = np.sum(np.transpose(gamma)[i])
                gammaDiagonalDot = np.diag(gammaDot / gammaTransposeSum)
                sigma[i] = np.diag(gammaDiagonalDot)
        return pi, mu, sigma
